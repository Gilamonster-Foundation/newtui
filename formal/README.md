...
| `ofView_consistent` | **proven** (`enc = id`) | With an injective default — the structural `Fingerprint` now on `main`, a `View` base plus a `Vec<String>` of extras — the observation half holds by construction, so the cheap guard (record the `View`, assert on a collision) cannot fire on a component that took the default. This changed a decision: two designs were about to bill that guard as closing the observation half outright. | **A refinement proof of `src/`.** It is a correspondence a reader checks by eye, and it was wrong once already: billed "checked against `component.rs:48-63`", the check missed that `Fingerprint` was a `String`. It describes the shipped default now; it does not verify the Rust, and no theorem here does. |
