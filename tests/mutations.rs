...
const MUTATIONS: &[Mutation] = &[
    Mutation {
        defect: "the linked host paints an empty counterpart anchor as an actual source row",
        file: "examples/support/linked.rs",
        from: "MappedLocation::Boundary(_) => None,",
        to: "MappedLocation::Boundary(_) => position.cursor,",
        expect_red: "tests::linked_host_uses_component_rows_and_never_marks_an_anchor_as_source",
        cargo_args: &["-p", "newtui-catalog", "--lib"],
    },
    Mutation {
        defect: "linked pane focus changes feed a rounded counterpart back into the original selection",
        file: "src/components/linked_panes.rs",
        from: "self.focus = side;",
        to: "self.focus = side; self.synchronize();",
        expect_red: "linked_cursors_follow_regions_without_losing_focus_roundtrips",
        cargo_args: &["--test", "linked_panes"],
    },
    Mutation {
        defect: "an arrow at a pane boundary changes the other selection despite no active movement",
        file: "src/components/linked_panes.rs",
        from: "if next == self.position(self.focus).cursor {",
        to: "if false {",
        expect_red: "saturated_navigation_preserves_the_other_selection_after_focus_changes",
        cargo_args: &["--test", "linked_panes"],
    },
    Mutation {
        defect: "a deleted counterpart boundary is misrepresented as an unrelated actual row",
        file: "src/components/linked_panes.rs",
        from: "MappedLocation::Boundary(target.start)",
        to: "MappedLocation::Row(target.start.saturating_sub(1))",
        expect_red: "missing_counterparts_keep_boundaries_instead_of_claiming_real_lines",
        cargo_args: &["--test", "linked_panes"],
    },
    Mutation {
        defect: "linked scroll offsets disappear from the default observable fingerprint",
        file: "src/components/linked_panes.rs",
        from: "                position.offset,",
        to: "                0,",
        expect_red: "linked_scroll_cursor_mode_and_correspondence_stay_in_the_fingerprint",
        cargo_args: &["--test", "linked_panes"],
    },
    Mutation {
        defect: "the numeric pane component queries the filesystem while handling a key",
        file: "src/components/linked_panes.rs",
        from: "        let position = self.position(self.focus);",
        to: "        let _ = std::fs::metadata(\".\"); let position = self.position(self.focus);",
        expect_red: "linked_panes_has_no_host_effect_entry_points",
        cargo_args: &["--test", "linked_panes"],
    },
    Mutation {
        defect: "live chart captions advance while the plotted history stays frozen",
        file: "examples/support/fixtures.rs",
        from: "let history = stream.histories();",
        to: "let history = DemoStream::default().histories();",
        expect_red: "tests::animated_numeric_pieces_change_real_chart_cells_and_pause_exactly",
        cargo_args: &["-p", "newtui-catalog", "--lib"],
    },
    Mutation {
        defect: "butterfly history repeats the latest samples instead of advancing through time",
        file: "src/widget/butterfly_history.rs",
        from: "let left = sample_at_age(tx, age);",
        to: "let left = sample_at_age(tx, 0);",
        expect_red: "butterfly_history_preserves_time_order_and_newest_alignment",
        cargo_args: &["--test", "butterfly_history"],
    },
    Mutation {
        defect: "both butterfly wings read transmit history while receive data is ignored",
        file: "src/widget/butterfly_history.rs",
        from: "let right = sample_at_age(rx, age);",
        to: "let _ = rx; let right = sample_at_age(tx, age);",
        expect_red: "butterfly_history_keeps_independent_wings_on_a_shared_scale",
        cargo_args: &["--test", "butterfly_history"],
    },
    Mutation {
    ...
    Mutation {
        defect: "a layout line target forgets its old-file line number",
        file: "src/widget/diff.rs",
        from: "            old: self.old,",
        to: "            old: None,",
        expect_red: "diff_row_targets_survive_blank_clipped_and_metadata_rows",
        cargo_args: &["--test", "diff_layout"],
    },
    Mutation {
        defect: "an old-side pane keeps added lines that exist only in the new file",
        file: "src/widget/diff.rs",
        from: "(Row::Source(source), DiffSide::Old) => source.old.is_some(),",
        to: "(Row::Source(source), DiffSide::Old) => true,",
        expect_red: "independent_diff_panes_keep_side_addresses_and_windows",
        cargo_args: &["--test", "diff_layout"],
    },
    Mutation {
        defect: "a fold target omits the last context entry it hides",
        file: "src/widget/diff.rs",
        from: "..sources[end - data.context - 1].index + 1",
        to: "..sources[end - data.context - 1].index",
        expect_red: "diff_folds_keep_addresses_without_retaining_source",
        cargo_args: &["--test", "diff_layout"],
    },
    Mutation {
    ...
    Mutation {
        ...
        from: "kind.output(sample, usize::from(pane.width))\n                        .expect(\"a pane owns a real widget\")",
        to: "WidgetOutput::new(vec![])",
        expect_red: "tests::bsp_host_renders_existing_widgets_inside_real_geometry",
        cargo_args: &["-p", "newtui-catalog", "--lib"],
    },
    Mutation {
        defect: "the catalog omits an exported layout primitive while component and widget inventories still match",
        file: "docs/CATALOG.md",
        from: "<!-- layout: bsp -->",
        to: "<!-- omitted layout: bsp -->",
        expect_red: "catalog_lists_every_component_export",
        cargo_args: &["--test", "catalog"],
    },
    Mutation {
    ...
    Mutation {
        defect: "widening the stored f32 ratio before multiplication loses a cell at decimal dial boundaries",
        file: "src/layout/bsp.rs",
        from: "let first_extent = (f32::from(available) * ratio).floor() as u16;",
        to: "let first_extent = (f64::from(available) * f64::from(ratio)).floor() as u16;",
        expect_red: "ten_available_cells_follow_the_stored_f32_proportion",
        cargo_args: &["--test", "layout"],
    },
    Mutation {
        defect: "BSP geometry reuses cell widths calculated for a 200-column area during a shrink",
        file: "src/layout/bsp.rs",
        from: "let first_extent = (f32::from(available) * ratio).floor() as u16;",
        to: "let first_extent = ((199.0 * ratio).floor() as u16).min(available);",
        expect_red: "bsp_ratio_resize_round_trip_and_paths_are_exact",
        cargo_args: &["--test", "layout"],
    },
    Mutation {
        defect: "a split path labels the first subtree as the second and no longer locates its divider",
        file: "src/layout/bsp.rs",
        from: "            path.push(false);",
        to: "            path.push(true);",
        expect_red: "bsp_ratio_resize_round_trip_and_paths_are_exact",
        cargo_args: &["--test", "layout"],
    },
    Mutation {
        defect: "BSP children consume the divider cell, overlapping the declared split border",
        file: "src/layout/bsp.rs",
        from: "    let divider = extent.min(1);",
        to: "    let divider = 0;",
        expect_red: "bounded_bsp_ratios_are_exhaustively_clean",
        cargo_args: &["--test", "layout"],
    },
    Mutation {
        defect: "ratio edits always store one half instead of the host's bounded selection",
        file: "src/layout/bsp.rs",
        from: "        *stored_ratio = ratio;",
        to: "        *stored_ratio = 0.5; let _ = ratio;",
        expect_red: "bounded_bsp_ratios_are_exhaustively_clean",
        cargo_args: &["--test", "layout"],
    },
    Mutation {
        defect: "changed-pane projection reports every pane, forcing reflow even when geometry is identical",
        file: "src/layout/bsp.rs",
        from: "        .filter(|id| before.get(id) != after.get(id))",
        to: "        .filter(|_| true)",
        expect_red: "changed_projection_reports_only_changed_added_or_removed_ids",
        cargo_args: &["--test", "layout"],
    },
    Mutation {
        defect: "Shrink loses its MIN_ROWS floor, so a modal can shrink past its own hint line",
        file: "src/layout/modal.rs",
        from: "self.requested = granted.saturating_sub(1).max(MIN_ROWS);",
        to: "self.requested = granted.saturating_sub(1);",
        expect_red: "never_below_the_minimum_and_never_granted_past_the_screen",
        cargo_args: &["--test", "modal"],
    },
    Mutation {
        defect: "a drag to zero or one row is requested as-is instead of floored at MIN_ROWS",
        file: "src/layout/modal.rs",
        from: "self.requested = rows.max(MIN_ROWS);",
        to: "self.requested = rows;",
        expect_red: "never_below_the_minimum_and_never_granted_past_the_screen",
        cargo_args: &["--test", "modal"],
    },
    Mutation {
        defect: "a second zoom leaves the modal filling the screen instead of restoring its height",
        file: "src/layout/modal.rs",
        from: "                    self.requested = previous;",
        to: "                    self.requested = FILL; let _ = previous;",
        expect_red: "zoom_fills_and_a_second_zoom_restores_the_prior_height",
        cargo_args: &["--test", "modal"],
    },
    Mutation {
        defect: "Grow steps from the request, banking rows past the clamped screen",
        file: "src/layout/modal.rs",
        from: "self.requested = granted.saturating_add(1).max(MIN_ROWS);",
        to: "self.requested = self.requested.saturating_add(1).max(MIN_ROWS);",
        expect_red: "grow_and_shrink_step_one_row_from_what_is_on_screen_and_leave_zoom",
        cargo_args: &["--test", "modal"],
    },
    Mutation {
        defect: "a drag while zoomed keeps the zoom, so the next zoom restores a stale height",
        file: "src/layout/modal.rs",
        from: "            SizeKey::To(rows) => {\n                self.before_zoom = None;",
        to: "            SizeKey::To(rows) => {",
        expect_red: "a_drag_sets_the_exact_height_floored_and_leaves_zoom",
        cargo_args: &["--test", "modal"],
    },
    Mutation {
        defect: "apply reports a change on every key, so the host relayouts on a no-op",
        file: "src/layout/modal.rs",
        from: "(self.requested != before).then_some(self.requested)",
        to: "{ let _ = before; Some(self.requested) }",
        expect_red: "a_shrink_at_the_minimum_changes_nothing",
        cargo_args: &["--test", "modal"],
    },
    Mutation {
        defect: "the modal host steps from its request rather than the height the screen granted",
        file: "examples/support/modal.rs",
        from: "self.size.apply(intent, self.granted());",
        to: "self.size.apply(intent, self.size.requested());",
        expect_red: "tests::modal_host_sizes_from_the_granted_height_and_zoom_round_trips",
        cargo_args: &["-p", "newtui-catalog", "--lib"],
    },
    Mutation {
